...
//                        list.get(i).isChoosed(list.get(i).isChoosed());
                        try{
                            list.get(i).isChoosed();
                        }catch(Exception e){
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if(list.get(position).getIsFirst()==1){
            ...
            holder.shop_checkbox.setChecked(list.get(position).isChoosed());
...
//        ImageLoader.getInstance().displayImage(split[0],holder.item_face);
        ...
        holder.item_checkbox.setChecked(list.get(position).isChoosed());
        ...
        holder.shop_checkbox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //先改变数据源中的shop_check
                list.get(position).setChoosed(holder.shop_checkbox.isChecked());

                for (int i=0;i<list.size();i++){
                    //如果是同一家商铺的 都给成相同状态
                    if(list.get(position).getSellerid()==list.get(i).getSellerid()){
                        //当前条目的选中状态 设置成 当前商铺的选中状态
                        list.get(i).setChoosed(holder.shop_checkbox.isChecked());
                    }
                }
                //刷新适配器
                notifyDataSetChanged();
        ...
        holder.item_checkbox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                list.get(position).setChoosed(holder.item_checkbox.isChecked());

                //反向控制商铺的shop_checkbox
                for (int i=0;i<list.size();i++){
                    for (int j=0;j<list.size();j++){
                        //如果两个商品是同一家店铺的 并且 这两个商品的item_checkbox选中状态不一样
                        if(list.get(i).getSellerid()==list.get(j).getSellerid() && !list.get(j).isChoosed()){
